...
double HelixLinear(double distance, double period, double offset, double max,double r1, double r2)
{
  const double pi = 3.1415926535897;
  double a=2*pi/period;
  ...
  double x=distance+offset;
  double y=r2*sin(x*a);
  double z=r1-r2*cos(x*a);

  double result=1-1/max*sqrt(pow(x,2)+pow(y,2)+pow(z,2));
...
double HelixLinear2(double distance, double period, double offset, double max, double r)
{
  const double pi = 3.1415926535897;
  double a=2*pi/period;
...
double HelixLogistic(double distance, double period, double offset, double max, double r)
{
  const double pi = 3.1415926535897;
  double a=2*pi/period;
  ...
  // double result=1-(1/max)*sqrt(pow(x,2)+pow(y,2)+pow(z,2));
  double result=1-(sqrt(pow(x,2)+2*pow(r,2)-2*pow(r,2)*cos(x*a)))/max;
  return (1/(1+exp(-result)))/0.731;
}
...
void Distance::read(ptree& pt)
{
  ...
  else if (func_name == "HelixLinear")
  {
    double_param_ptr maxparam(   new Parameter<double>(string(name+" Max"),    pt.get_child("Max")));
    double_param_ptr periodparam(new Parameter<double>(string(name+" Period"), pt.get_child("Period")));
    double_param_ptr offsetparam(new Parameter<double>(string(name+" Offset"), pt.get_child("Offset")));
    double_param_ptr r1param(new Parameter<double>(string(name+" R1"), pt.get_child("R1")));
    double_param_ptr r2param(new Parameter<double>(string(name+" R2"), pt.get_child("R2")));
    params["Max"]    = maxparam;
    params["Period"] = periodparam;
    params["Offset"] = offsetparam;
    params["R1"] = r1param;
    params["R2"] = r2param;
    max_distance = params["Max"]->getLimHigh();
    distFunc = bind(&HelixLinear, _1, 
		    boost::ref(params["Period"]->getValue()), 
		    boost::ref(params["Offset"]->getValue()), 
		    boost::ref(params["Max"]->getValue()),
		    boost::ref(params["R1"]->getValue()),
		    boost::ref(params["R2"]->getValue()));

  }
  else if (func_name == "HelixLinear2")
  {
    double_param_ptr maxparam(   new Parameter<double>(string(name+" Max"),    pt.get_child("Max")));
    double_param_ptr periodparam(new Parameter<double>(string(name+" Period"), pt.get_child("Period")));
    double_param_ptr offsetparam(new Parameter<double>(string(name+" Offset"), pt.get_child("Offset")));
    double_param_ptr rparam(new Parameter<double>(string(name+" R"), pt.get_child("R")));
    params["Max"]    = maxparam;
    params["Period"] = periodparam;
    params["Offset"] = offsetparam;
    params["R"] = rparam;
    max_distance = params["Max"]->getLimHigh();
    distFunc = bind(&HelixLinear2, _1, 
		    boost::ref(params["Period"]->getValue()), 
		    boost::ref(params["Offset"]->getValue()), 
		    boost::ref(params["Max"]->getValue()),
		    boost::ref(params["R"]->getValue()));

  }
  else if (func_name == "HelixLogistic")
  {
    double_param_ptr maxparam(   new Parameter<double>(string(name+" Max"),    pt.get_child("Max")));
    double_param_ptr periodparam(new Parameter<double>(string(name+" Period"), pt.get_child("Period")));
    double_param_ptr offsetparam(new Parameter<double>(string(name+" Offset"), pt.get_child("Offset")));
    ...
    params["R"] = rparam;
    max_distance = params["Max"]->getLimHigh();
    distFunc = bind(&HelixLogistic, _1, 
		    boost::ref(params["Period"]->getValue()), 
		    boost::ref(params["Offset"]->getValue()), 
		    boost::ref(params["Max"]->getValue()),
		    boost::ref(params["R"]->getValue()));
...
void Distance::write(ptree& pt)
{
  ...
  else if (func_name == "HelixLinear")
  {
    ...
    ptree& period_node = distance_node.add("Period", "");
    ptree& offset_node = distance_node.add("Offset", "");
    ptree& R1_node = distance_node.add("R1", "");
    ptree& R2_node = distance_node.add("R2", "");
    
    params["Max"]->write(max_node, mode->getPrecision());
    params["Period"]->write(period_node, mode->getPrecision());
    params["Offset"]->write(offset_node, mode->getPrecision());
    params["R1"]->write(R1_node, mode->getPrecision());
    params["R2"]->write(R2_node, mode->getPrecision());
  ...
  else if (func_name == "HelixLinear2")
  {
    ...
    ptree& period_node = distance_node.add("Period", "");
    ptree& offset_node = distance_node.add("Offset", "");
    ptree& R_node = distance_node.add("R", "");
    
    params["Max"]->write(max_node, mode->getPrecision());
    params["Period"]->write(period_node, mode->getPrecision());
    params["Offset"]->write(offset_node, mode->getPrecision());
    params["R"]->write(R_node, mode->getPrecision());
